...
- **Risk calibration and GPU benchmarks** — [Risk calibration](risk-calibration.md) documents heuristic thresholds, ECE/Platt scaling, and benchmark validation (run via `experiment/scripts/calibrate_risk.py`; see [Validation Report](validation-report.md)). [GPU benchmarks](gpu-benchmarks.md) describes how to measure overhead with `--perf strict` and where to find results in the report.
...
- **Viewing data** — Two paths: (A) Hosted React dashboard + local API (`corevital serve`); your data stays on your machine. (B) Enterprise: use native sinks (Datadog, Prometheus, W&B) in your config; no dashboard required. See [README — Visualizing your Data](../README.md#visualizing-your-data-two-viewing-paths).
...
Optional: `pip install "CoreVital[serve]"` and run `corevital serve` to expose your local SQLite DB to the [hosted dashboard](https://main.d2maxwaq575qed.amplifyapp.com); your data never leaves your machine. Enterprise users can skip the dashboard and use Datadog, Prometheus, or W&B sinks (see [Production deployment](production-deployment.md)).
